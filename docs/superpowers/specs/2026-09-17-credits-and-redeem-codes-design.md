...
**补记（2026-09-17，最终全分支审查发现）：上下文滚动压缩（`chat_service._maybe_summarize` → `summarize_history`）也应明确列在这份豁免清单里。**

它的性质与上面四项不同，必须写清楚而不是默默略过：前面四项的触发与成本都与对话长度无关，而这一项**随对话变长而变贵**——它用**用户自己的 `ModelConfig`**、在用户的会话上发起一次真实的 provider 调用。原始设计漏了它，于是它既不在豁免清单里，也没有被计费，属于"既没说收费也没说不收费"的灰区。

**决定：豁免（不计费）**，理由是它由平台策略自动触发、用户无法选择是否发生，把它计入会让账单出现用户无法解释的条目——与上面四项同一条原则。但代价要说明白：长会话的用户会得到一部分"平台代付"的压缩成本，且这块成本随会话增长。

若将来要改为计费，正确做法不是在 `_charge_quota_if_enabled` 那类入口上打补丁，而是把它纳入 §5.1 的统一结算入口（让它挂到某一轮的用户消息上），否则又会回到"存在一条只消费不计费的路径"。
