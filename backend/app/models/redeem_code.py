...
**只存 HMAC-SHA256 哈希，不存明文。** 兑换码是不记名凭证，等价于现金：明文入库
...
是 AB12CD 开头的那张"）。诚实的位置：6 位前缀本身给了掌握库的攻击者 30 bit
已知明文，裸哈希下暴力破解一张码的量级是单卡 GPU 天；改用 HMAC-SHA256
（见 ``app.credits.hash_code``），密钥来自服务端 pepper（``REDEEM_CODE_PEPPER``，
留空回落到由 ``JWT_SECRET`` 派生）——**只有数据库 dump、没有应用层秘密的
攻击者拿不到任何可验证的候选**，这正是哈希存储要防的对手。
"""
